...
// ===============================
// Leonardo Andrés Estepa Castro
//                                                                                                                              ~Hawks-L11
// ===============================

function agregarReserva(edificio, sala, franja, docente) {
  ...
  if (ocupado) {
    alert("Esa sala ya está ocupado en esa franja");
    return;
  }

  reservas.push({ edificio, sala, franja, docente });
...
// registro ocupacion salas bloques franjas horarios docentes
document.getElementById("btnReservar").addEventListener("click", () => {
  ...
  agregarReserva(edificio, sala, franja, docente);
});

// control por edificio
function filtrarPorEdificio(nombreEdificio) {
  return reservas.filter(r => r.edificio === nombreEdificio);
}

// 
function filtrarPorDocente(nombreDocente) {
  return reservas.filter(r => r.docente === nombreDocente);
}
